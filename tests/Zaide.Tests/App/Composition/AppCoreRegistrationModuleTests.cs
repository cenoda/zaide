...
public sealed class AppCoreRegistrationModuleTests
{
    ...
    private static readonly string[] M6bPlusDirectMarkers =
    {
        "AddSingleton<ISettingsService, SettingsService>()",
        "AddSingleton<IFileTreeService, FileTreeService>()",
        "AddSingleton<FileTreeViewModel>()",
    ...
    [Fact]
    public void ProgramSource_StillDeclaresM6bPlusRegistrationsDirectly()
    {
        var programSource = ReadRepoFile("src/App/Composition/Program.cs");

        foreach (var marker in M6bPlusDirectMarkers)
        {
            Assert.Contains(marker, programSource);
        }

        // No other M6 feature modules exist yet.
        Assert.DoesNotContain("AddZaideSettings", programSource);
