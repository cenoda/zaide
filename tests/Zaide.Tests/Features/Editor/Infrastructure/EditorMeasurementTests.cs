...
public class EditorMeasurementTests
{
    ...
    public async Task OpenEditSaveRestore_Completes_AndRestoresSavedContent()
    {
        ...
        var sourceShaBefore = EditorMeasurementSeam.Sha256Hex(fixture);
        ...
        Assert.Equal(sourceShaBefore, EditorMeasurementSeam.Sha256Hex(fixture));
    ...
    public async Task LargeDocumentLoad_OpensGeneratedEightMibFixture()
    {
        ...
        var expectedSha = EditorMeasurementSeam.Sha256Hex(largePath);
        var sample = await EditorMeasurementSeam
        ...
        Assert.True(sample.ElapsedMs > 0);
        Assert.Equal(expectedSha, EditorMeasurementSeam.Sha256Hex(largePath));
    }

    [Fact]
    public void CreateTabManager_UsesProductionFileServiceAndWorkspace()
    {
        var tabs = EditorMeasurementSeam.CreateTabManager();
        Assert.NotNull(tabs);
    ...
    public async Task MeasurementRunner_WritesEvidence_WhenOutputEnvSet()
    {
        ...
        foreach (var area in areas)
        {
            if (area is "editor")
            {
                ...
                {
                    var warmUp = await EditorMeasurementSeam
                        .MeasureOpenEditSaveRestoreAsync(editorFixture, sampleNumber: 0);
                    Assert.Equal("pass", warmUp.Status);
                }

                var samples = new EditorMeasurementSample[sampleCount];
                for (var i = 1; i <= sampleCount; i++)
                {
                    samples[i - 1] = await EditorMeasurementSeam
    ...
    private static void EnsureLargeFileFixture(string largePath)
    {
        ...
        if (File.Exists(largePath)
            && EditorMeasurementSeam.Sha256Hex(largePath) == expectedSha)
        {
            return;
        }
    ...
    private static void WriteAreaEvidence(string outputRoot, string area, EditorMeasurementSample[] samples)
    {
