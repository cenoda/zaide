...
#### M7c completion record

**Scope executed:** Mechanical rehome of Debugging presentation only into
`src/Features/Debugging/Presentation/` and matching
`tests/Zaide.Tests/Features/Debugging/Presentation/`. Namespace
`Zaide.ViewModels` / `Zaide.Views` → `Zaide.Features.Debugging.Presentation` for the 18
production paths (debug panel/session/stack projection ViewModels, editor
breakpoint/IP markers and projection, breakpoint and instruction-pointer margins/
operations, DebugPanel view). Matching 9 tests rehomed (including
`TestDebugPanelFactory` and `TestEditorBreakpointFactory`). No renames beyond
path/namespace. Namespace collisions under `Features/Debugging` required
`Avalonia.Application.Current` and `global::Zaide.Features.Workspace.Domain.Workspace`
in moved presentation sources/tests only.

Public full-name baseline rewrote 16 type names (count still 348). Architecture
inventory updated for folder/namespace truth (ViewModels 23 files / 40 types;
Views 26 files / 28 types; Features 255 files; Presentation namespace 19/16/3).
CONVENTIONS + OVERVIEW truthful current-tree notes. No DI registration/lifetime,
visibility, constructor signature, or behavior changes. FindingId allowlist
unchanged (9). M8+ SourceControl and later features remain.
...
**Next:** stop after M7c; do not start M8+ without authorization.
...
**Scope executed:** Mechanical rehome of SourceControl only into
`src/Features/SourceControl/{Domain,Contracts,Application,Infrastructure,Presentation}/`
and matching `tests/Zaide.Tests/Features/SourceControl/...`. Namespaces
`Zaide.Models` / `Zaide.Services` / `Zaide.ViewModels` / `Zaide.Views` →
`Zaide.Features.SourceControl.*` for the 28 production paths. Matching 13 tests
rehomed (Integration, Domain, Application, Infrastructure, Presentation,
`SourceControlTestFactory`). No assertion rewrites; durable feature-oriented
test namespaces only. Residual R61-V02/V07 edges kept via inventory path-scoped
detection under Features + allowlist MatchKey rewrites:
...
| `R61-AL-LOC-SourceControlDiffTabService` | same |

Mechanical collision fixes only: `global::Zaide.Features.Workspace.Domain.Workspace`,
`Avalonia.Application.Current`, and `AppPushResult` alias for LibGit2Sharp
`PushResult` ambiguity. No DI registration/lifetime, visibility, constructor
signature, or behavior changes. FindingId allowlist unchanged (9). Public count
348. M9+ Terminal and later features remain.
...
**FindingId allowlist:** unchanged (9 entries; three SourceControl MatchKeys path-rewritten). **Public count:** 348.

**Next:** stop after M8; do not start M9+ without authorization.
...
**Scope executed:** Mechanical rehome of Terminal only into
`src/Features/Terminal/{Contracts,Application,Infrastructure,Presentation}/`
and matching `tests/Zaide.Tests/Features/Terminal/...`. Namespaces
`Zaide.Services` / `Zaide.ViewModels` / `Zaide.Views` →
`Zaide.Features.Terminal.*` for the 24 production paths. Matching 14 tests
rehomed (Application, Infrastructure, Presentation). No assertion rewrites;
durable feature-oriented test namespaces only. Residual R61-V05 factory-to-
presentation edges kept via inventory path-scoped detection under Features +
allowlist MatchKey rewrites:
...
No DI registration/lifetime, visibility, constructor signature, factory
redesign, or behavior changes. FindingId allowlist unchanged (9). Public count
348. M10+ Townhall and later features remain.
...
**FindingId allowlist:** unchanged (9 entries; two Terminal MatchKeys path-rewritten). **Public count:** 348.

**Next:** stop after M9; do not start M10+ without authorization.
