...
| M3 qualification smoke | **NO-GO** (`M3_QUALIFICATION_EVIDENCE.md`): latest authorized fresh session `m3q-20260724T075320Z-939e94cf` — preflight (DNS/slirp/inner egress) **GO** before credential read+delete; write-capable `--approval-mode yolo`; Qwen **once** under then-locked **24** turns / **240s** wall; TC-T01 rename **verified** (`FetchData` 0 / `RetrieveData` 11; host build/test 0); **`qwen_exit=53`** (`FatalTurnLimitedError` turn limit) → dual GO fails; balance-before/after USD **3.94** / **3.94** (delta **USD 0.00**); fixed parent reap recorded real inner exit **4**; full finalization **completed**. Prior historical `m3q-20260724T072341Z-8f567943` (24/120s, exit 55, finalization incomplete). **Active policy 240 turns / 800s wall** (extended single-smoke exception for one future retry). |
...
| M3 extended single-smoke policy remediation (2026-07-24) | **Complete** (`M3_EXTENDED_SINGLE_SMOKE_POLICY_REMEDIATION_EVIDENCE.md`): active lock **`--max-session-turns 240`**, **`--max-wall-time 800s`** (was 24/240s); overall inner budget **880s**; explicitly approved **one** future qualification retry only; historical 24/240s session `m3q-20260724T075320Z-939e94cf` unchanged; **not** a qualification retry; future retry still requires new one-shot key + explicit grant |
...
| Locked smoke turn / time / spend ceilings | `--max-session-turns 240`, `--max-wall-time 800s`, smoke **USD 1**, Phase 16 cumulative **USD 3**. Latest smoke (`m3q-20260724T075320Z-939e94cf`) ran under then-locked 24/240s (exit 53). Prior historical `m3q-20260724T072341Z-8f567943` under then-24/120s (exit 55). Prior `m3q-20260724T060109Z-45dd1c5f` under then-12/120s. Historical `m3q-20260724T054307Z-481ad1de` used 60s/12 (unchanged). |
...
| Next external grants | New qualification grant + **new** dedicated sub-key one-shot file for **one** authorized extended retry; keep write-capable lock + **240** turns + **800s** wall + fixed reap/finalization path; GO only on exit 0 **and** verified TC-T01 rename; **not** authorization for multiple attempts |
...
qualification smoke session `m3q-20260724T075320Z-939e94cf` (fresh, one only)
NO-GO: 24-turn / 240s (then-locked; historical record unchanged); Qwen exit 53
(turn limit) after verified rename; preflight before key; finalization complete;
spend delta USD 0.00; inner exit 4. Prior historical
`m3q-20260724T072341Z-8f567943` (24/120s, exit 55). **Active policy 240 turns
/ 800s wall** (extended single-smoke exception for one future retry). Candidate
remains not qualified. No second attempt under that grant. No retry in extended
single-smoke policy slice. No M4.*
