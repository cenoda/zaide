...
| Locked smoke turn / time / spend ceilings | `--max-session-turns 12`, `--max-wall-time 60s`, smoke **USD 1**, Phase 16 cumulative **USD 3**. Used by session `m3q-20260724T054307Z-481ad1de` |
...
| Next external grants | New qualification grant + **new** dedicated sub-key one-shot file if retry is authorized separately; keep write-capable lock + reap path; GO only on exit 0 **and** verified TC-T01 rename |
...
yolo path verified TC-T01 rename but Qwen exit 55 (60s wall); spend balance
delta USD 0.00; post-exit finalization completed. Candidate remains not
