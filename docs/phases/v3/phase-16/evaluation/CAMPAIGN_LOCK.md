...
| M3 qualification smoke | **NO-GO** (`M3_QUALIFICATION_EVIDENCE.md`): latest authorized fresh session `m3q-20260724T072341Z-8f567943` — preflight (DNS/slirp/inner egress) **GO** before credential read+delete; write-capable `--approval-mode yolo`; Qwen **once** under **24** turns / **120s**; TC-T01 rename **verified** (`FetchData` 0 / `RetrieveData` 11; host build/test 0); **`qwen_exit=55`** (`FatalBudgetExceededError` wall 120s) → dual GO fails; balance-before USD 3.95 (after unavailable); fixed parent reap recorded real inner exit **4**; full finalization incomplete. Prior latest `m3q-20260724T060109Z-45dd1c5f` (12-turn, exit 53). Active policy 24 turns / 120s. |
...
NO-GO: 24-turn / 120s; Qwen exit 55 after verified rename; preflight before key;
finalization incomplete; inner exit 4. Prior `m3q-20260724T060109Z-45dd1c5f`.
**Active policy 24 turns / 120s wall.**
Candidate remains not qualified. No second attempt. No M4.*
