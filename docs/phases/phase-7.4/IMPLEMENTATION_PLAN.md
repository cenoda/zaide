...
| **M3** ✅ | Verified and stabilized the full stage/unstage/commit loop after M1/M2. Confirmed (via an automated end-to-end replay through the real `SourceControlViewModel` + real read/refresh/diff/mutation seams against throwaway repositories) that: selection persists across post-mutation refresh (re-selected by path, diff reloaded), the diff surface stays coherent after stage/unstage/commit, the branch/status/change projection stays truthful, and repeated stage/unstage cycles do not degrade (no duplicate/orphaned entries, no stale `StatusMessage`). `IGitRepositoryService`, `ISourceControlSnapshotOrchestrator`, and `IFileDiffService` are unchanged. **Regression fixed (view-layer):** clicking the panel's refresh or commit button crashed the app with `InvalidOperationException` — `SourceControlPanel` piped the `RoutedEventArgs` event stream straight into the Unit-parameterized `RefreshCommand`/`CommitCommand` via `InvokeCommand`, forwarding the `EventPattern` as the command parameter. Fixed minimally by projecting each event stream to `Unit.Default` before `InvokeCommand`. This was invisible to VM-level tests (which invoke commands directly); a dedicated wiring regression test now guards it. **Smoke-loop truth-sync:** the literal loop *create → stage → modify → view diff → commit* does **not** make the file disappear from both lists — git commits only the staged snapshot, so the post-stage modification correctly remains as an unstaged `Modified` entry. The file disappears from both lists only for the clean sequence (*create → stage → commit* with no post-stage edit) or *stage → unstage → restage → commit*. The panel reflects git truth in every case. | Build + all tests green (`dotnet build Zaide.slnx --no-restore`, `dotnet test Zaide.slnx --no-build`, 815 pass). New automated M3 coverage in `tests/Zaide.Tests/Integration/SourceControlMutationFlowTests.cs` (8 tests): clean create→stage→diff→commit disappearance; stage→modify→commit truthful leftover; unstage→restage→commit; repeated stage/unstage stability; selection persistence across stage refresh; empty-message rejection (no commit created); nothing-staged rejection; missing-identity commit-specific error surface (environment-aware). Wiring regression coverage in `tests/Zaide.Tests/Views/SourceControlPanelCommandWiringTests.cs` (3 tests): un-projected `InvokeCommand` throws the parameter-type mismatch; `Unit`-projected refresh/commit pipelines execute without throwing. |
...
- [x] Tests pass: `dotnet test Zaide.slnx --no-build` (815 pass)
