...
| **M1** ✅ | Implemented `IGitMutationService` with `Stage` and `Unstage` (returning `StageResult`; `Commit`/`CommitResult` deferred to M2). Registered in DI (`src/Program.cs`). Replaced the body of `SourceControlViewModel.StageFileCommand` / `UnstageFileCommand` to call the mutation seam via `ReactiveCommand.CreateFromTask` + `Task.Run`, resolving the repository root per-operation via `IGitRepositoryService.Discover`. Refresh runs unconditionally after each mutation attempt; failures surface via `StatusMessage`. The existing placeholder tests were rewritten as seam-backed tests at both service and ViewModel level. Updated stale XML doc comments on `FileChange.cs` and `SourceControlViewModel.cs`. | Service tests (`tests/Zaide.Tests/Services/GitMutationServiceTests.cs`): stage an untracked file, stage a modified file, unstage a file, no-op stage of already-staged, no-op unstage of already-unstaged, stage against a non-existent repository returns failure. ViewModel tests (`tests/Zaide.Tests/ViewModels/SourceControlViewModelTests.cs`): stage/unstage call the mutation seam then refresh; stage/unstage failure surfaces `StatusMessage`; no-repository guards the mutation call. All 795 tests pass (`dotnet test Zaide.slnx --no-build`). |
| **M2** ✅ | Wired `CommitCommand` to call `IGitMutationService.Commit` via `ReactiveCommand.CreateFromTask` + `Task.Run` with per-operation repository discovery. Implemented `CommitResult` return type with truthful validation: empty commit message (no git call), nothing staged (VM guard + service guard), missing git identity (service-level `BuildSignature` check), and repo/IO failure (catch-all). Added `CommitError` property to `SourceControlViewModel` (distinct from `StatusMessage`/`LastRefreshError`/`LastRefreshStatus`) with a red `TextBlock` surface in `SourceControlPanel`. On success: clears `CommitMessage`, clears `CommitError`, refreshes. On failure: sets `CommitError`, does not overload refresh-state properties. `ApplyResult` clears `CommitError` on successful refresh to prevent stale errors. Service tests (5 new): empty message, nothing staged, valid commit, missing identity (environment-aware), non-existent repo. ViewModel tests (4 new): success clears message/error and refreshes, nothing staged guards mutation, empty message guards mutation, failure surfaces `CommitError`. | All 804 tests pass (`dotnet test Zaide.slnx --no-build`). |
| **M3** ✅ | Verified and stabilized the full stage/unstage/commit loop after M1/M2. Confirmed (via an automated end-to-end replay through the real `SourceControlViewModel` + real read/refresh/diff/mutation seams against throwaway repositories) that: selection persists across post-mutation refresh (re-selected by path, diff reloaded), the diff surface stays coherent after stage/unstage/commit, the branch/status/change projection stays truthful, and repeated stage/unstage cycles do not degrade (no duplicate/orphaned entries, no stale `StatusMessage`). `IGitRepositoryService`, `ISourceControlSnapshotOrchestrator`, and `IFileDiffService` are unchanged. **Regression fixed (view-layer):** clicking the panel's refresh or commit button crashed the app with `InvalidOperationException` — `SourceControlPanel` piped the `RoutedEventArgs` event stream straight into the Unit-parameterized `RefreshCommand`/`CommitCommand` via `InvokeCommand`, forwarding the `EventPattern` as the command parameter. Fixed minimally by projecting each event stream to `Unit.Default` before `InvokeCommand`. This was invisible to VM-level tests (which invoke commands directly); a dedicated wiring regression test now guards it. **Regression fixed (open-folder wiring):** opening a folder via the file-tree "Open Folder..." header called `FileTreeViewModel.OpenFolderCommand` directly, bypassing the workspace/Source-Control sync that only `MainWindowViewModel.OpenFolderCommand` (Ctrl+O) performed, so `Workspace.WorkspacePath` stayed null and Source Control reported "No repository" for a real repo. Fixed by driving the workspace-path update + Source Control refresh from a `FileTreeViewModel.RootPath` subscription in `MainWindowViewModel.Activate()`, so every open-folder entry point stays truthful; guarded by a new regression test. **Smoke-loop truth-sync:** the literal loop *create → stage → modify → view diff → commit* does **not** make the file disappear from both lists — git commits only the staged snapshot, so the post-stage modification correctly remains as an unstaged `Modified` entry. The file disappears from both lists only for the clean sequence (*create → stage → commit* with no post-stage edit) or *stage → unstage → restage → commit*. The panel reflects git truth in every case. | Build + all tests green (`dotnet build Zaide.slnx --no-restore`, `dotnet test Zaide.slnx --no-build`, 816 pass). New automated M3 coverage in `tests/Zaide.Tests/Integration/SourceControlMutationFlowTests.cs` (8 tests): clean create→stage→diff→commit disappearance; stage→modify→commit truthful leftover; unstage→restage→commit; repeated stage/unstage stability; selection persistence across stage refresh; empty-message rejection (no commit created); nothing-staged rejection; missing-identity commit-specific error surface (environment-aware). Wiring regression coverage in `tests/Zaide.Tests/Views/SourceControlPanelCommandWiringTests.cs` (3 tests): un-projected `InvokeCommand` throws the parameter-type mismatch; `Unit`-projected refresh/commit pipelines execute without throwing. Open-folder regression coverage in `tests/Zaide.Tests/MainWindowViewModelTests.cs` (`OpeningFolderViaFileTreeDirectly_SyncsWorkspaceAndRefreshesSourceControl`). |
...
- [x] The user can stage files from the Source Control surface (M1)
- [x] The user can unstage files from the Source Control surface (M1)
- [x] The user can create a local commit with truthful validation and failure behavior (M2)
- [x] Post-mutation UI refresh keeps branch/status/change data truthful (M3)
- [x] `IGitRepositoryService`, `ISourceControlSnapshotOrchestrator`, and `IFileDiffService`
      are unchanged (grep-verified)
- [x] The three old placeholder tests are gone; new seam-backed tests exist for
      stage, unstage, and commit at both the service and ViewModel levels
- [x] Stale XML doc comments updated: `FileChange.cs` no longer says "Used for static/demo
...
- [x] Tests pass: `dotnet test Zaide.slnx --no-build` (816 pass)

## Exact Next Step

None. Phase 7.4 is complete and closes Roadmap V1. Any later Git scope must be
planned separately rather than added to this completed implementation plan.
