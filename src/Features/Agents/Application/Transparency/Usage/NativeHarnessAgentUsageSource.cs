...
internal sealed class NativeHarnessAgentUsageSource : IAgentUsageBackendEvidenceSource
{
    private readonly AgentUsageBackendEvidenceSourceWriter _writer;

    public NativeHarnessAgentUsageSource(AgentUsageBackendEvidenceSourceWriter writer)
    {
    ...
    public AgentUsageCaptureResult Submit(AgentUsageCaptureRequest request)
    {
        ...
        if (!CanExpose(request.Kind))
        {
            return new AgentUsageCaptureResult(
                ...
                reason: "native-harness-cannot-expose-kind");
        }
